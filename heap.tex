...
\begin{Interfaz}
  ...
  INCOMPLETO
  \textbf{se explica con}: \tadNombre{colaPrior($\alpha$)}.

  
  \section*{Operaciones básicas de \tadNombre{colaPrior($\alpha$)}}

  \InterfazFuncion{Vacia}{}{colaPrior($\alpha$)}
  {$res \igobs vacia()$}
  [$\Theta()$]
  [genera la cola de prioridades vacía]

  \InterfazFuncion{Encolar}{\Inout{r}{\tadNombre{colaPrior($\alpha$)}}, \In{a}{$\alpha$}{}
...
\begin{Representacion}
  ...
  \begin{Estructura}{red}[estr]
    \begin{Tupla}[estr]
      ...
      \tupItem{compus}{dicc(id, conj(compu))}%
    \end{Tupla}
