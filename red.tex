...
(def?($i_1$, obtener($i$, e.vecinos)) $\land$ def?($i_2$, obtener($i$, e.vecinos)) $\land$ $i_1 \neq i_2$ $\impluego$ obtener($i_1$, obtener($i$, e.vecinos)) $\neq$ obtener($i_2$, obtener($i$, e.vecinos)))
   ...
   \AbsFc[estr]{red}[e]{
     r : red / computadoras(r) = juntar(claves(e.vecinos), e.interfaces) $\land$ \\ 
...
\begin{algorithmic}[1]
  \Procedure{AgregarComputadora($r$ : red, $c$ : compu)}{}
...
\caption{Conectar}
\begin{algorithmic}[1]
  \Procedure{Conectar($r$ : red, $c_1$ : compu, $c_2$ : compu, $i_1$ : interfaz, $i_2$ : interfaz)}{}
...
\caption{Conectadas?}
\begin{algorithmic}[1]
  \Procedure{Conectadas?($r$ : red, $c_1$ : compu, $c_2$ : compu)}{}
